...
public final class AsynchronousExecutor<P, T, C, E extends Throwable> {
                ...
                case STAGE_1_COMPLETE:
                    try {
                        ...
                        for (final C callback : callbacks) {
                            cc ++;
                            if (cc > Math.floor(callbacks.size()*(SpigotWorldConfig.entityMaxTickPercent/100))){break;}
                            if (callback == this) {
                            ...
                            ompExecutor.execute(new Runnable(){
                                ...
                                public void run() {
                                    try {
                                        ...
                                        Logger.getLogger(AsynchronousExecutor.class.getName()).log(Level.SEVERE, null, ex);
                                    }
                                }});
                        }ompExecutor.waitForExecution();
                        System.out.println(SpigotWorldConfig.entityMaxTickPercent);
                        /*
    ...
    CallBackProvider<P, T, C, E> provider;
    Queue<Task> finished = new ConcurrentLinkedQueue<Task>();
    Map<P, Task> tasks = new HashMap<P, Task>();
    ...
    public void finishActive() throws E {
        Queue<Task> finished = this.finished;
        int cc = 0;
        //System.out.println("O size:"+finished.size()+"T size:"+finished.size()*SpigotWorldConfig.chunkeMaxTickPercent/100);
        while (!finished.isEmpty()) {
            cc++;
            if (cc > Math.floor(finished.size()*(SpigotWorldConfig.chunkeMaxTickPercent/100))){break;}
