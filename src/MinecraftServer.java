...
public abstract class MinecraftServer implements Runnable, ICommandListener, IAsyncTaskHandler, IMojangStatistics {
    ...
    public void B() {
        ...
        for (int ompI = 0; ompI < 4; ompI++) {//Runtime.getRuntime().availableProcessors()
            final int ompJ = ompI;
            final MinecraftServer th = this;
            ompExecutor.execute(new Runnable(){
                @Override
                public void run() {
                    if (ompJ == 0) {
                        methodProfiler.c("levels");
                        ...
                        if (ticks % 20 == 0) {
                            for (int i = 0; i < th.getPlayerList().players.size(); ++i) {
                                EntityPlayer entityplayer = (EntityPlayer) th.getPlayerList().players.get(i);
        ...
        */
        ...
        //System.out.println("p size:"+this.p.size());
