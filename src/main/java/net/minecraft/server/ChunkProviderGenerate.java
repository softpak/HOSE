...
public class ChunkProviderGenerate implements IChunkProvider {
    ...
    double d1, d2, d3, d4, d5, d6, d7, d8, d10, d11, d12, d13, d15, d16;
    public void a(int i, int j, ChunkSnapshot chunksnapshot) {
        this.B = this.m.getWorldChunkManager().getBiomes(this.B, i * 4 - 2, j * 4 - 2, 10, 10);
        this.a(i * 4, 0, j * 4);

        Aparapi.range(4).forEach(gid_k -> {
            int l = gid_k * 5;
            int i1 = (gid_k + 1) * 5;
            Aparapi.range(4).forEach(gid_j1 -> {
                int k1 = (l + gid_j1) * 33;
                int l1 = (l + gid_j1 + 1) * 33;
                int i2 = (i1 + gid_j1) * 33;
                int j2 = (i1 + gid_j1 + 1) * 33;
                Aparapi.range(32).forEach(gid_k2 -> {
                    double d0 = 0.125D;
                    d1 = this.p[k1 + gid_k2];
                    d2 = this.p[l1 + gid_k2];
                    d3 = this.p[i2 + gid_k2];
                    d4 = this.p[j2 + gid_k2];
                    d5 = (this.p[k1 + gid_k2 + 1] - d1) * d0;
                    d6 = (this.p[l1 + gid_k2 + 1] - d2) * d0;
                    d7 = (this.p[i2 + gid_k2 + 1] - d3) * d0;
                    d8 = (this.p[j2 + gid_k2 + 1] - d4) * d0;
                    Aparapi.range(8).forEach(gid_l2 -> {
                        double d9 = 0.25D;
                        d10 = d1;
                        d11 = d2;
                        d12 = (d3 - d1) * d9;
                        d13 = (d4 - d2) * d9;
                        Aparapi.range(4).forEach(gid_i3 -> {
                            double d14 = 0.25D;
                            d15 = (d11 - d10) * d14;
                            d16 = d10 - d15;
                            Aparapi.range(4).forEach(gid_j3 -> {
                                if ((d16 += d15) > 0.0D) {
                                    chunksnapshot.a(gid_k * 4 + gid_i3, gid_k2 * 8 + gid_l2, gid_j1 * 4 + gid_j3, Blocks.STONE.getBlockData());
                                } else if (gid_k2 * 8 + gid_l2 < this.r.q) {
                                    chunksnapshot.a(gid_k * 4 + gid_i3, gid_k2 * 8 + gid_l2, gid_j1 * 4 + gid_j3, this.s.getBlockData());
                                }
                            });
                            d10 += d12;
                            d11 += d13;
    ...
    public void a(int i, int j, ChunkSnapshot chunksnapshot, BiomeBase[] abiomebase) {
        ...
        Aparapi.range(16).forEach(gid_k -> {
            Aparapi.range(16).forEach(gid_l -> {
                BiomeBase biomebase = abiomebase[gid_l + gid_k * 16];

                biomebase.a(this.m, this.h, chunksnapshot, i * 16 + gid_k, j * 16 + gid_l, this.t[gid_l + gid_k * 16]);
            });
        });
        /*
        for (int k = 0; k < 16; ++k) {
    ...
    public Chunk getOrCreateChunk(int i, int j) {
        ...
        Aparapi.range(abyte.length).forEach(gid_k -> abyte[gid_k] = (byte) this.B[gid_k].id);
        /*
    ...
    private void a(int i, int j, int k) {
        ...
        Aparapi.range(5).forEach(gid_j1 -> {
            Aparapi.range(5).forEach(gid_k1 -> {
                float f2 = 0.0F;
                float f3 = 0.0F;
                float f4 = 0.0F;
                byte b0 = 2;
                BiomeBase biomebase = this.B[gid_j1 + 2 + (gid_k1 + 2) * 10];

                for (int l1 = -b0; l1 <= b0; ++l1) {
                    for (int i2 = -b0; i2 <= b0; ++i2) {
                        BiomeBase biomebase1 = this.B[gid_j1 + l1 + 2 + (gid_k1 + i2 + 2) * 10];
                ...
                Aparapi.range(33).forEach(gid_j2 -> {
                    double d4 = ((double) gid_j2 - d3) * (double) this.r.l * 128.0D / 256.0D / d2;

                    if (d4 < 0.0D) {
                        d4 *= 4.0D;
                    }

                    double d5 = this.e[hl] / (double) this.r.d;
                    double d6 = this.f[hl] / (double) this.r.c;
                    double d7 = (this.d[hl] / 10.0D + 1.0D) / 2.0D;
                    double d8 = MathHelper.b(d5, d6, d7) - d4;

                    if (gid_j2 > 29) {
                        double d9 = (double) ((float) (gid_j2 - 29) / 3.0F);
    ...
    public void getChunkAt(IChunkProvider ichunkprovider, int i, int j) {
        ...
        blockposition = new BlockPosition(k, 0, l);
        ...
        if (this.r.s) {
            Aparapi.range(this.r.t).forEach(gid_k1 -> {
        ...
        Aparapi.range(16).forEach(gid_k1 -> {
            Aparapi.range(16).forEach(gid_l1 -> {
                BlockPosition blockposition1 = this.m.q(blockposition.a(gid_k1, 0, gid_l1));
