...
public class NoiseGenerator3Handler {
    ...
    public NoiseGenerator3Handler(Random random) {
        ...
        Aparapi.range(256).forEach(gid_i -> {
            int j = random.nextInt(256 - gid_i) + gid_i;
            int k = this.f[gid_i];

            this.f[gid_i] = this.f[j];
            this.f[j] = k;
            this.f[gid_i + 256] = this.f[gid_i];
        });
    ...
    public void a(double[] adouble, double d0, double d1, int i, int j, double d2, double d3, double d4) {
        ...
        Aparapi.range(j).forEach(gid_l -> {
            double d5 = (d1 + (double) gid_l) * d3 + this.c;
            Aparapi.range(i).forEach(gid_i1 -> {
                double d6 = (d0 + (double) gid_i1) * d2 + this.b;
