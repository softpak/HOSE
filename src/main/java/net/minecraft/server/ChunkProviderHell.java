...
public class ChunkProviderHell implements IChunkProvider {
    ...
    double d1, d2, d3, d4, d10, d11, d12, d13, d15;
    public void a(int i, int j, ChunkSnapshot chunksnapshot) {
        ...
        Aparapi.range(b0).forEach(gid_j1 -> {
            Aparapi.range(b0).forEach(gid_k1 -> {
                Aparapi.range(16).forEach(gid_l1 -> {
                    double d0 = 0.125D;
                    d1 = this.n[((gid_j1 + 0) * i1 + gid_k1 + 0) * b1 + gid_l1 + 0];
                    d2 = this.n[((gid_j1 + 0) * i1 + gid_k1 + 1) * b1 + gid_l1 + 0];
                    d3 = this.n[((gid_j1 + 1) * i1 + gid_k1 + 0) * b1 + gid_l1 + 0];
                    d4 = this.n[((gid_j1 + 1) * i1 + gid_k1 + 1) * b1 + gid_l1 + 0];
                    double d5 = (this.n[((gid_j1 + 0) * i1 + gid_k1 + 0) * b1 + gid_l1 + 1] - d1) * d0;
                    double d6 = (this.n[((gid_j1 + 0) * i1 + gid_k1 + 1) * b1 + gid_l1 + 1] - d2) * d0;
                    double d7 = (this.n[((gid_j1 + 1) * i1 + gid_k1 + 0) * b1 + gid_l1 + 1] - d3) * d0;
                    double d8 = (this.n[((gid_j1 + 1) * i1 + gid_k1 + 1) * b1 + gid_l1 + 1] - d4) * d0;
                    Aparapi.range(8).forEach(gid_i2 -> {
                        double d9 = 0.25D;
                        d10 = d1;
                        d11 = d2;
                        d12 = (d3 - d1) * d9;
                        d13 = (d4 - d2) * d9;
                        Aparapi.range(4).forEach(gid_j2 -> {
                            double d14 = 0.25D;
                            d15 = d10;
                            double d16 = (d11 - d10) * d14;
                            Aparapi.range(4).forEach(gid_k2 -> {
                                IBlockData iblockdata = null;

                                if (gid_l1 * 8 + gid_i2 < k) {
                                ...
                                int l2 = gid_j2 + gid_j1 * 4;
                                int i3 = gid_i2 + gid_l1 * 8;
                                int j3 = gid_k2 + gid_k1 * 4;
    ...
    public Chunk getOrCreateChunk(int i, int j) {
        ...
        Aparapi.range(abyte.length).forEach( gid_k -> abyte[gid_k] = (byte) abiomebase[gid_k].id);
        /*
    ...
    private double[] a(double[] adouble, int i, int j, int k, int l, int i1, int j1) {
        ...
        Aparapi.range(i1).forEach( gid_l1 -> {
            adouble1[gid_l1] = Math.cos((double) gid_l1 * 3.141592653589793D * 6.0D / (double) i1) * 2.0D;
            double d2 = (double) gid_l1;

            if (gid_l1 > i1 / 2) {
                d2 = (double) (i1 - 1 - gid_l1);
            }

            if (d2 < 4.0D) {
                d2 = 4.0D - d2;
                adouble1[gid_l1] -= d2 * d2 * d2 * 10.0D;
            }
        });
        /*
        for (l1 = 0; l1 < i1; ++l1) {
        ...
        Aparapi.range(l).forEach( gid_l1 -> {
            Aparapi.range(j1).forEach( gid_i2 -> {
                double d3 = 0.0D;
                Aparapi.range(i1).forEach( gid_j2 -> {
                    double d4 = 0.0D;
                    double d5 = adouble1[gid_j2];
                    ...
                    if (gid_j2 > i1 - 4) {
                        d9 = (double) ((float) (gid_j2 - (i1 - 4)) / 3.0F);
                        d4 = d4 * (1.0D - d9) + -10.0D * d9;
                    }

                    if ((double) gid_j2 < d3) {
                        d9 = (d3 - (double) gid_j2) / 4.0D;
